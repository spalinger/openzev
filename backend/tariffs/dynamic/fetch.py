...
#: Maximum lookback for any scheduled refresh; older history needs a backfill.
RECOVERY_DAYS = 14
...
def refresh_source(source: DynamicTariffSource, *, backfill: bool = False, now: datetime | None = None) -> FetchResult:
    ...
    if not source.supports_range:
        ...
        # Transport windows include yesterday through tomorrow in UTC. This
        # deliberately covers complete Swiss publication days as well as the
        # UTC billing window, including runs just after local midnight.
        today = now.replace(hour=0, minute=0, second=0, microsecond=0)
        horizon = today - timedelta(days=RECOVERY_DAYS)
        if backfill:
            start = today - timedelta(days=BACKFILL_DAYS)
        else:
            resume_from = source.recovery_from or source.covers_to or source.last_success_at
            start = min(today - timedelta(days=1), max(resume_from or today, horizon))
        range_start = start
    ...
    try:
        ...
        previous = source.recovery_from or resume_from
        if range_start is not None and previous is not None and previous < range_start:
