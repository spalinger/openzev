...
class TestEvidenceProtection:
    ...
    @pytest.mark.parametrize("cursor", ["covers_to", "last_success_at", "recovery_from"])
    def test_a_stale_scheduled_cursor_is_bounded_but_not_forgotten(self, cursor):
        old = datetime(2024, 1, 1, tzinfo=UTC)
        source = make_source(**{cursor: old})
        ...
        assert fetch.call_args.args[1].start == datetime(2026, 1, 18, tzinfo=UTC)
        source.refresh_from_db()
        assert source.recovery_from == old
    ...
    def test_a_window_that_could_not_be_fetched_is_reported_not_dropped(self):
        source = make_source()
        source.covers_to = datetime(2026, 1, 20, tzinfo=UTC)
        source.save()
        with mock.patch("tariffs.dynamic.fetch.MAX_WINDOW_DAYS", 12), mock.patch(
