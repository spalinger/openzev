...
class EmptyAndMalformedTests(SimpleTestCase):
    ...
    def test_v2_units_that_cannot_be_billed_are_reported_rather_than_dropped_silently(self):
        ...
        series = parse_versioned_response(fixture("vse_v2_grid"), api_version="v2_0_0", tariff_type="grid", tariff_name="standard")
...
class V2ProductIdentityTests(SimpleTestCase):
    def test_a_blank_identity_refuses_disjoint_products(self):
        payload = fixture("vse_v2_grid")
        import copy
        later = copy.deepcopy(payload["prices"][0])
        later["start_timestamp"] = "2027-02-01T00:00:00+01:00"
        later["end_timestamp"] = "2027-02-01T00:15:00+01:00"
        later["grid"]["tariff_name"] = "seasonal"
        payload["prices"].append(later)
        with self.assertRaisesRegex(DynamicTariffResponseError, "require a product name"):
            parse_versioned_response(payload, api_version="v2_0_0", tariff_type="grid")

    def test_missing_embedded_name_is_reported_as_missing(self):
        payload = fixture("vse_v2_grid")
        del payload["prices"][0]["grid"]["tariff_name"]
        with self.assertRaisesRegex(DynamicTariffResponseError, "omits.*product name"):
            parse_versioned_response(payload, api_version="v2_0_0", tariff_type="grid", tariff_name="standard")
