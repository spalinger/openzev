...
class DynamicTariffTransferTests(TestCase):
    ...
    def test_reusing_a_disabled_source_returns_warnings(self):
        zev, source = self._zev_with_dynamic_tariff()
        raw = export_and_clear(zev)
        source.enabled = False
        source.save(update_fields=["enabled"])
        result = import_archive(io.BytesIO(raw), owner=self.importer)
        self.assertTrue(any("different settings" in warning for warning in result["warnings"]))
...
class DynamicSourceImportTests(TestCase):
    ...
    def test_a_blank_v2_source_is_rejected_even_without_tariffs(self):
        from zev.transfer.importer import _Collector, _dynamic_source_for

        with self.assertRaisesRegex(ValueError, "require a product name"):
            _dynamic_source_for({"dynamic_source": {
                "label": "Blank", "url": "https://example.test/prices", "api_version": "v2_0_0",
                "tariff_type": "grid", "tariff_name": "",
            }}, _Collector())
