...
links. The backfill is atomic and writes bounded batches.
...
and then see their committed evidence, or proceed after a rollback leaves no
evidence. These tests run against a PostgreSQL CI service; the full SQLite
suite alone cannot verify row locks. Global source locks can delay concurrent
