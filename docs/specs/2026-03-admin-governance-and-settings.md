...
- KPI row: three `StatCard` in `.grid.grid-3` — ZEVs, Participants, and Total revenue (`formatChf` from `frontend/src/lib/numbers.ts`, without thousands grouping, e.g. `CHF 1234.56`) as the single dark `accent` card. Pending/failed email counts are deliberately not repeated here — they live in the Email Statistics breakdown below.
