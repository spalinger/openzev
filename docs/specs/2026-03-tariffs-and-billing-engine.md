...
- Percentage tariff responses include `percentage_base_summary` (null for
  ...
  Plain list and series routes batch these summaries per source; paginated
  lists include applicable grid versions outside the returned page.
...
windows are conservative, including tariffs that produced no line items.
