...
|---|---|---|---|---|
| Dashboard | `frontend/src/pages/DashboardPage.tsx` (cards in `frontend/src/components/dashboard/`: `BalanceChart`, `ConsumptionSplitCard`, `EnergyFlowCard`, `HourlyProfileCard`, `ParticipantInvoicesCard`, `ParticipantTableCard`) | manager `/dashboard` (Energy balance) + participant `/` | `queryKeys.metering.dashboardSummary({…})` + `queryKeys.invoices.list()` (participant-only, top-level parallel fetch) + `queryKeys.metering.hourlyProfile(…)` (participant always; manager when a participant is selected) | none — read-only. Manager: ZEV-wide KPI row (5 `StatCard`s from `zev_totals`) → `EnergyFlowCard` → `BalanceChart` (both keep `selectedParticipantId` highlight/filter) → `ParticipantTableCard` (row click sets the participant filter) → `HourlyProfileCard` when a participant is selected and profile data is available. Participant: 3 `StatCard`s → `EnergyFlowCard` (requires `current_participant_id`) → `ConsumptionSplitCard` → `HourlyProfileCard` → `ParticipantInvoicesCard` (role-scoped `fetchInvoices` filtered to `approved|sent|paid` with `pdf_url`, details + PDF actions). Header eyebrow is the scope line: selected ZEV name for owners/admins, the participant's community name (`zev_name`, only with a single membership, `zev_count === 1`) from `/auth/me` for participants; blank with no selection or several memberships. Report exports live on Reports; participant invoice PDF actions remain on the dashboard. |
| Reports | `frontend/src/pages/ReportsPage.tsx` | `/reports` (participant branch also served at `/me/statement`) | none (queries + mutations) | `downloadAnnualStatement({year})` (participant single PDF), async whole-ZEV ZIP via export jobs (`AnnualStatementsExportCard`: `createAnnualStatementsExport` → poll `fetchExportJob` while queued/running → `downloadAnnualStatementsExport`, with partial/failed/expired states, a ZEV/year-switch guard and reload restore), `downloadFinancialSummary({year, zev_id?})` — `zev_id` for admin/owner, omitted for participant (backend scopes by participant). Role branches: admin/owner ZIP card + financial summary; participant single PDF + financial summary. Layout: scope-name header eyebrow (selected ZEV for admin/owner, participant community `zev_name` for participants), one shared year selector (`aria-label`, no visible label, disabled while any download or export is busy, defaults to the last completed year, recomputed per render), `YearDownloadCard`s (participant statement, financial summary) plus `AnnualStatementsExportCard` (admin/owner ZIP) in a `grid grid-2`, `ReportsEmptyState` guard distinguishing no-ZEV (`pages.reports.noZev*`) from stale/no selection (`pages.reports.selectZev*`). |
| Participant statement | `frontend/src/pages/ReportsPage.tsx` | `/me/statement` (participant-only canonical host; `/reports` serves all three roles) | none (mutations only) | same participant downloads as Reports (`downloadAnnualStatement`, `downloadFinancialSummary` scoped by participant) |
...
| Invoice detail | `frontend/src/pages/InvoiceDetailPage.tsx` | `/billing/invoices/:invoiceId` (`AppRoutes.tsx:203`) | `queryKeys.invoices.detail(invoiceId)` = `['invoices', 'detail', invoiceId]` (`queryKeys.ts:25`) | `generate-pdf` → set `pdf_url`; embed `PdfPreview src={pdfUrl}` |
...
- `frontend/src/pages/ReportsPage.tsx` + `frontend/src/features/reports/YearDownloadCard.tsx` + `frontend/src/features/reports/ReportsEmptyState.tsx` + `frontend/src/features/reports/AnnualStatementsExportCard.tsx` (admin/owner ZIP: async prepare → poll → download, partial/failed/expired states, ZEV/year switch guard, reload restore) — new `/reports` page (lazy route, `ProtectedRoute allowedRoles={['admin','zev_owner','participant']}`) with a shared page-level year selector, a `grid grid-2` card row, `YearDownloadCard` deduping the instant-download-card pattern, and `ReportsEmptyState` for the ZEV guard; `DashboardPage` slim-down (download controls, mutations, `annualStatementYear` state, `dashboard.quickStart` i18n key removed; header eyebrow → scope line). `Layout.tsx` adds `nav.reports` + `ReportsIcon` between Imports and the Setup group, rendered inside the manage-only nav block (participants reach their statement download via `/me/statement`, not the Reports entry; the `/reports` route itself still allows their role so bookmarks work). Route tree lives in `frontend/src/components/AppRoutes.tsx` (extracted from `App.tsx` for the guard-matrix test); `frontend/src/i18n/locales/{de,en,fr,it}.ts` move the download strings under `pages.reports.*`. `AppFooter.tsx`, `LanguageSelector.tsx`, `StatCard.tsx`, `ActionMenu.tsx`, `ConfirmDialog.tsx`/`FormModal.tsx`/`EmailLogsModal.tsx`, `PeriodSelector.tsx`, `RawMeteringTable.tsx`, `EnergyFlowChart.tsx`, `frontend/src/lib/chartTokens.ts` + feature folders.
...
- Pages: `DashboardPage` (slimmed: KPI → flow → exceptions → charts; no downloads; scope-line eyebrow), `ReportsPage` (`/reports`, `YearDownloadCard` dedup + `AnnualStatementsExportCard`, role-branched downloads),
