...
| Preview fix | `preview-pdf-template` can return `application/pdf` (real WeasyPrint bytes); admin editor shows it in an `<iframe>` via object URL; invoice detail uses `pdf_url` only as the stored-artifact signal, then embeds the authenticated `/pdf/` blob in a paper frame; this branch adds the participant annual documents (Annual Statement + Tax Overview tabs) on `ReportsPage` (contract embed deferred) |
...
resets the relevant PDFs. Admins and owners use the whole-ZEV ZIP export.

Contract embedding (`GET /api/v1/zev/participants/{pk}/contract-pdf/`) remains
deferred until a participant detail surface can host the viewer.
...
### 7.7 PdfPreview component and object-URL lifecycle

**Files:** `frontend/src/components/PdfPreview.tsx` (presentational iframe/link + opt-in action row) and `frontend/src/lib/usePdfObjectUrl.ts` (authenticated blob lifecycle used by invoice detail and the participant annual documents).
...
interface PdfPreviewProps {
  ...
  openInNewTabFetcher?: () => Promise<Blob>  // blob supplier for the new tab (fetch or already-loaded)
  actions?: ReactNode       // opt-in row above the viewer beside the new-tab link
}
```

- `PdfPreview` renders an object-URL iframe in `.pdf-frame`. With `actions`, Download and Open in new tab appear above it; otherwise Open stays below. The caller owns fetching and URL cleanup.
- `usePdfObjectUrl(fetcher, enabled)` returns `{url, blob, loading, error}`. It rejects declared non-PDF types, clears stale state when the fetcher changes, aborts superseded browser requests, and revokes URLs on replacement, disable, or unmount. `AdminPdfTemplatesPage` retains its separate debounce and last-successful-frame behavior.
- `openInNewTabFetcher` supplies a fresh or cached blob. `PdfPreview` reserves the tab during the click, then navigates it to a separate object URL, revoked after 60 seconds. A failed fetch falls back to `src`.
...
| `fetchInvoicePdfBlob(invoiceId, signal?)` | GET | ``/invoices/invoices/{id}/pdf/`` | Authenticated blob fetch via axios client (auth + 401 refresh); replaces same-origin ``/media/`` fetches that break when ``DEBUG=False`` drops ``static()`` media serving; the signal aborts superseded preview fetches |
| `generateInvoicePdf(id)` | POST | `/invoices/invoices/{id}/generate-pdf/` | reused by detail page when `pdf_url` is null |
| `downloadAnnualStatement({year, participant_id?, zev_id?}, signal?)` | GET | `/invoices/invoices/annual-statement/` | Returns the generated PDF blob. The participant Annual Statement tab loads it on entry and reuses it for Download/Open; the function never triggers a browser download itself. |
| `downloadFinancialSummary({year, zev_id?, participant_id?}, signal?)` | GET | `/invoices/invoices/financial-summary/` | Returns the generated PDF blob. The participant Tax Overview tab loads it on first selection the same way; the manager card saves it via `downloadBlob`. |
...
- `frontend/src/lib/usePdfObjectUrl.ts`
- (dropped — see §7.8)
...
- `ReportsPage.tsx` and `ParticipantYearDocuments.tsx` — participants see annual document tabs at `/me/statement` and `/reports`; managers retain their tax overview card at `/reports`. `AppRoutes.tsx` guards both routes. Report labels live in all four locale files.
- `BillingStatementsPage.tsx` and `AnnualStatementsExportCard.tsx` — admin/owner ZIP preparation, polling, download, and recovery after reload or selection changes at `/billing/statements`.
...
- Playwright `npm run screenshots` — 35 guide captures, including the embedded participant statement.
...
- CSP and embeds: object-URL iframes work with the app CSP. Invoice and annual-document URLs are revoked on replacement or unmount; the template editor keeps its last successful frame during rendering.
...
- [x] Template preview uses the same rendering pipeline, template version, token output and PDF profile as issued documents (no HTML facsimile; parity, not byte-equality); invoice detail embeds its stored artifact via authenticated blob fetch; participant annual documents generate on entry/first-selection through the shared `usePdfObjectUrl` lifecycle (contract embed deferred — §§2,6.2).
