...
| `tariff_name` | `CharField(120)`, blank for v1 | Operator product; v2 requires a nonblank identity |
| `enabled` | `BooleanField` | disabled sources retain evidence; no automatic initial backfill or scheduled refresh |
...
`UniqueConstraint(url, api_version, tariff_type, tariff_name)` defines unique
identity. Immutability is an application rule, not a database constraint. Model saves and the API refuse identity edits; Django admin makes
those fields read-only after creation. `CheckConstraint(dynamic_exact_url_no_range)`
rejects `request_mode=exact_url` with `supports_range=true`. New v2 sources must
carry a nonblank product. A legacy blank v2 source can be disabled, but cannot
be re-enabled, fetched or used for billing; create a named replacement. Raw
`QuerySet.update()` bypasses model validation and is not a configuration API.
...
backfill and live recording intentionally protect whole applicable windows.
The tariff edit/delete guard also remains conservative across tariffs sharing a
source within the same ZEV; `tariff_id_snapshot` records provenance, not a claim
that only that tariff could have consumed a source interval.
...
A blank v2 creation request resolves exactly one discovered product and stores
its embedded name. Multiple products require a choice; an empty endpoint
requires the expected name to be typed. A persistent blank v2 identity is
refused, including archive imports. Each fetched v2 component must include the
required `tariff_name` and match the configured product. Missing and different
names produce distinct errors; disjoint publication periods do not permit
mixing products. An unnamed v2 creation must re-probe to resolve its product,
even if the resolved natural key is later reused.
...
| Mode | Window |
|---|---|
| Scheduled | Resume `recovery_from`, otherwise stored extent/last success; clamp every scheduled lookback to 14 days, no later than yesterday; end at UTC today + 2 days |
...
the original rows, including billed rows. This does not authorize changes to
prices or coverage within a billed interval. Evidence ranges are merged into
sorted disjoint spans once, then checked with bisection. Independent valid
...
writes through `store_points` under the source lock. Direct ORM/bulk writes to
points are unsupported; admin exposes them read-only. Cross-table price-band
exclusion likewise belongs to supported tariff validation, not a DB trigger.
...
When creation, admin or import creates a new enabled source, it queues
...
included in the four-hour fan-out if enabled. A disabled source receives no
automatic backfill; an explicit admin/API/command fetch remains available.
...
V2's discovered product name is persisted; if an empty endpoint cannot resolve
it, an explicit nonblank name is required. Missing or different response names
are refused.
...
- **`POST /tariffs/dynamic-sources/`**: owner or admin creation. The server
  ...
  the probe found but cannot bill. An exact key reuse skips the probe and
  returns no new warnings; reuse after resolving an unnamed v2 request retains
  the probe warnings.
...
- **`DynamicSourceFormModal`** — creation is a two-step discovery flow. Step 1
  ...
  the endpoint. V2 product names are shown directly; an empty v2 endpoint
  requires manual product entry. V1 offers an optional
...
when newly created on import (no backfill is queued for it). Reused sources
keep destination settings; mismatches and disabled/failing state produce
nonfatal warnings displayed in the transfer result. Price points and the
...
| `invoices/test_tariff_overview.py::TariffOverviewDynamicTariffTests` | Unfetched tariff prints an unavailable row, fetched average with its footnote, percentage-tariff footnote and amount |
| `tariffs/test_dynamic_components.py` | Per-version aggregate composition, recursive v2 expansion, `dso` excludes electricity, `integrated` certain/possible split, plain/unknown types empty |
| `tariffs/test_vse_import.py` | Dynamic grid candidate is importable, no-URL and storage-refund blocks, the missing-product warning, version-aware aggregate warnings (preview names certain + possible-extra, apply upgrades to the probed version's exact list), `is_free` correctness, source get-or-create + probe + version-aware reuse after probing, real probe-point initialization with successful status/timestamps, unreachable-URL error, post-commit initial-backfill enqueueing only after a successful new-source tariff write, auto-detection of the probed API version (not hardcoded v1.0.5), and a new source's dropped-unit warnings reaching the apply result |
| `tariffs/test_dynamic_source_api.py` | Authenticated role access, global list and picker fields |
...
| Module | Coverage |
|---|---|
| `tests/dynamic-sources.test.ts` | Source/energy-type helpers; discovery; paginated list; manual create; bounded history query; fetch queueing; typed clear request; typed source delete (the aggregate expansion is served by the backend — see `test_dynamic_components.py`) |
| `tests/dynamic-source-form-modal.test.ts` | Two-step rendering, version-only choices, absence of provider choices, and discovered component/product selection |
| `tests/vse-tariff-import.test.ts` (extended) | A dynamic candidate is selectable, offers no billing-mode choice, can be the pre-selected recommendation |
| `tests/tariff-form-mapping.test.ts` (extended) | `dynamic_source` round-trips through the form, is dropped when billing mode is not energy, defaults to blank |

Frontend `tests/import-warning-modals.test.ts` verifies that applied VSE
warnings stay under their created tariff and transfer warnings remain visible
until the result is closed. `tests/invoice-actions-hook.test.ts` covers
structured single/bulk generation refusals. The source wizard test covers
required product entry for empty v2 endpoints.
