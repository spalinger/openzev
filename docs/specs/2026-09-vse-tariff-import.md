...
operational warning to the candidate result. Each created item exposes these
as `dynamic_source_warnings: string[]`; the frontend retains every warning
directly under that tariff in the result list. The preview alone cannot
supply apply-time version and reused-source warnings.
...
the price cell. V2 must resolve to one nonblank product during probing. If
the endpoint is empty, the operator must provide the expected product name
...
interface VseTariffImportResult {
    created: Array<{
        ...
        dynamic: boolean; dynamic_source_warnings: string[]
    }>
...
### Backend — `backend/tariffs/test_vse_import.py`
